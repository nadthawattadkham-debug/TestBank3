...
export default function App() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (u) {
      // In simulator mode, any password works or we assume simple "1234" for convenience
      const authenticatedUser = { id: u.id, username: u.username, name: u.name, role: u.role };
      setCurrentUser(authenticatedUser);
      localStorage.setItem('gas_exam_active_user', JSON.stringify(authenticatedUser));
      setLoginUsername('');
      setLoginPassword('');
    } else {
      setLoginError('ไม่พบรายชื่อผู้ใช้ดังกล่าวในระบบชีต Users (หรือกรอกผิด)');
    }
  };
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setRegSuccess('ลงทะเบียนผู้ใช้ใหม่ลงชีต Users สำเร็จ! ย้ายไปหน้าล็อกอินได้เลยครับ');
    setTimeout(() => {
